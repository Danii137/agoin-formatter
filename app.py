...
st.set_page_config(page_title="AGOIN Formateador", page_icon="🏢", layout="wide")
...
{logo_html}
<h1>📄 AGOIN Formateador</h1>
<p style="color: #2d8b73;">Especificaciones exactas aplicadas</p>
<div>
<span class="feature-badge">✓ 1.36cm × 1.36cm</span>
<span class="feature-badge">✓ Square wrap</span>
<span class="feature-badge">✓ Centered</span>
</div>
</div>
""", unsafe_allow_html=True)
...
def is_title_level_1(text, style_name):
    ...
    if text.isupper() and len(text) < 100 and not re.match(r'^\d+[.-]', text):
        return True
    keywords = ['ÍNDICE', 'CONCLUSIÓN', 'INTRODUCCIÓN', 'RESUMEN']
    return any(kw in text.upper() for kw in keywords) and len(text) < 60
...
def is_title_level_2(text, style_name):
    ...
    if re.match(r'^\d+(\.\d+)*[.-]\s', text):
        return True
    return len(text) < 80 and text.endswith(':') and not text.isupper()

def is_list_item(text):
    return re.match(r'^[•\-–—]\s', text) or re.match(r'^[a-z]\)|[ivxIVX]+\)|\d+\)', text)

def create_footer_exact_specs(section):
    """
    PIE CON LOGO - ESPECIFICACIONES EXACTAS
    Text Wrapping: Square, Both sides, 0.32cm
    Size: 1.36cm × 1.36cm
    Position: Centered (Column), -0.07cm below Paragraph
    """
    ...
    # PÁRRAFO 1: Logo + Texto empresa
    para_empresa = footer.paragraphs[0]
    para_empresa.alignment = WD_ALIGN_PARAGRAPH.RIGHT
    para_empresa.paragraph_format.space_before = Pt(0)
    para_empresa.paragraph_format.space_after = Pt(0)
    ...
    try:
        ...
        # Añadir imagen con tamaño EXACTO: 1.36cm × 1.36cm
        inline_shape = run_logo.add_picture(logo_stream, width=Cm(1.36), height=Cm(1.36))

        # Convertir inline a anchor con wrapping SQUARE
        inline = inline_shape._inline
        cx, cy = inline.extent.cx, inline.extent.cy

        # Obtener graphic XML de forma segura
            ...
            try:
                if hasattr(graphic_el, 'xml'):
                    ...
                    if isinstance(graphic_xml, bytes):
                        ...
                        graphic_xml = str(graphic_xml)
                else:
                    graphic_xml = str(graphic_el)
            except:
                raise Exception("No se pudo extraer graphic XML")

        # XML del anchor con especificaciones EXACTAS
        # Distance: 0.32cm = 115200 EMUs (1cm = 360000 EMUs, 0.32cm = 115200)
        # Position V: -0.07cm = -25200 EMUs
        anchor_xml = (
            '<wp:anchor xmlns:wp="http://schemas.openxmlformats.org/drawingml/2006/wordprocessingDrawing" '
            'distT="0" distB="0" distL="115200" distR="115200" '
            ...
            '<wp:posOffset>-25200</wp:posOffset>'
            '</wp:positionV>'
    ...
    para_contacto.alignment = WD_ALIGN_PARAGRAPH.RIGHT
...
def apply_agoin_format_final(input_doc, project_title, project_location, is_text_only=False):
    ...
    for section in output_doc.sections:
        ...
        add_green_header_paragraph(header_title, project_title if project_title else "[TÍTULO]", is_bold=True)
        header_location = header.add_paragraph()
        add_green_header_paragraph(header_location, project_location if project_location else "[DIRECCIÓN]", is_bold=False)
        header_location.paragraph_format.space_before = Pt(1)
        create_footer_exact_specs(section)
...
with tab2:
    ...
    if pasted_text and len(pasted_text.strip()) > 10:
        ...
        st.success(f"✅ {len(pasted_text)} caracteres")
        ...
        if st.button("✨ Generar", use_container_width=True, type="primary"):
            with st.spinner("🔄 Aplicando formato AGOIN..."):
                try:
                    ...
                    st.download_button("📄 Descargar DOCX", data=buffer, file_name=f"AGOIN_{source_name}.docx", mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document", use_container_width=True)
                except Exception as e:
                    st.error(f"❌ {str(e)}")
    except Exception as e:
        st.error(f"❌ {str(e)}")
...
st.markdown("---")
st.markdown('<p style="text-align: center; color: #1a5c4d; font-weight: 600;">AGOIN v13.0 Final • Especificaciones exactas</p>', unsafe_allow_html=True)
