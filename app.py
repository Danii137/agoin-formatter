import streamlit as st
from docx import Document
from docx.shared import Pt,Cm,RGBColor
from docx.enum.text import WD_ALIGN_PARAGRAPH
from docx.oxml.shared import OxmlElement,qn
import io,re,base64
from io import BytesIO

st.set_page_config(page_title="AGOIN Formatter",page_icon="🏢",layout="wide")
LOGO_BASE64=""

st.markdown("""<style>
.stApp{background:linear-gradient(to top left,#1a5c4d 0%,#2d8b73 20%,#fff 100%)}
.main-header{background:rgba(255,255,255,0.95);padding:2rem;border-radius:15px;text-align:center;margin-bottom:2rem;box-shadow:0 4px 6px rgba(0,0,0,0.1)}
.main-header h1{color:#1a5c4d;margin:0}
.stButton>button{background:linear-gradient(135deg,#1a5c4d 0%,#2d8b73 100%);color:white;font-weight:600;border-radius:10px;padding:0.75rem 2rem;border:none}
.stButton>button:hover{transform:translateY(-2px);box-shadow:0 4px 12px rgba(26,92,77,0.3)}
.doc-preview{background:white;padding:3rem 2.5rem;border-radius:8px;box-shadow:0 8px 16px rgba(0,0,0,0.15);max-height:600px;overflow-y:auto;font-family:'Century Gothic',sans-serif;margin:2rem auto;max-width:900px}
.doc-header{background:#1a5c4d;color:white;padding:8px 12px;text-align:right;font-size:11px;font-weight:bold;margin-bottom:15px;border-radius:4px}
.doc-title{font-size:14px;font-weight:bold}
.doc-location{font-size:11px;margin-top:3px;opacity:0.95}
.doc-content{font-size:10pt;line-height:1.6;text-align:justify}
.doc-content h1{background:#1a5c4d;color:white;padding:8px 12px;font-size:11pt;font-weight:bold;text-align:right;margin:20px 0 10px 0;border-radius:4px}
.doc-content h2{font-size:11pt;font-weight:bold;color:#000;margin:15px 0 8px 0}
.doc-content p{margin:8px 0}
.doc-content table{width:100%;border-collapse:collapse;margin:15px 0}
.doc-content table th,.doc-content table td{border:1px solid #ddd;padding:8px;font-size:9pt}
.doc-content table th{background:#f5f5f5;font-weight:bold}
.doc-footer{border-top:1px solid #ddd;margin-top:30px;padding-top:15px;display:flex;justify-content:space-between;align-items:center;font-size:9pt}
.doc-footer-text{text-align:right;line-height:1.4}
.doc-footer-text .company{font-weight:bold;font-size:9pt}
.doc-footer-text .contact{font-size:7pt;color:#666;margin-top:2px}
.stats-card{background:white;padding:1.5rem;border-radius:10px;box-shadow:0 2px 8px rgba(0,0,0,0.1);text-align:center}
.stats-number{font-size:2rem;font-weight:bold;color:#1a5c4d}
.stats-label{color:#666;font-size:0.9rem;margin-top:0.5rem}
</style>""",unsafe_allow_html=True)

st.markdown(f'<div class="main-header"><img src="data:image/jpeg;base64,{LOGO_BASE64}" width="70"><h1>AGOIN Formatter</h1><p style="color:#2d8b73">Formateador profesional de documentos</p></div>',unsafe_allow_html=True)

def add_green_header_paragraph(p,t,b=True):
    r=p.add_run(t)
    r.font.name='Century Gothic'
    r.font.size=Pt(11)
    r.font.bold=b
    r.font.color.rgb=RGBColor(255,255,255)
    p.alignment=WD_ALIGN_PARAGRAPH.RIGHT
    s=OxmlElement('w:shd')
    s.set(qn('w:fill'),'1a5c4d')
    p._element.get_or_add_pPr().append(s)
    p.paragraph_format.space_before=Pt(0)
    p.paragraph_format.space_after=Pt(0)

def extract_project_info_from_text(t):
    i={'title':'','location':''}
    l=t.split('\n')[:10]
    for line in l:
        line=line.strip()
        if line and(line.isupper()or any(k in line.upper()for k in['ACTA','INFORME','MEMORIA','PROPUESTA'])):
            if len(line)<100 and not i['title']:
                i['title']=line
                break
    tj=' '.join(l)
    lp=re.search(r'(?:CALLE|AVENIDA|AVDA|C/|AVENIDA DE)[^\n]{0,150}',tj,re.IGNORECASE)
    if lp:i['location']=lp.group(0).strip()
    return i

def extract_project_info(d):
    i={'title':'','location':''}
    if hasattr(d,'paragraphs'):
        for p in d.paragraphs[:5]:
            t=p.text.strip()
            if t and(t.isupper()or any(k in t.upper()for k in['ACTA','INFORME','MEMORIA','PROPUESTA'])):
                if len(t)<100:
                    i['title']=t
                    break
        txt=' '.join([p.text for p in d.paragraphs[:10]])
        lp=re.search(r'(?:CALLE|AVENIDA|AVDA|C/|AVENIDA DE)[^\n]{0,150}',txt,re.IGNORECASE)
        if lp:i['location']=lp.group(0).strip()
    return i

def is_title_level_1(t,s):
    if s=='Heading 1':return True
    if t.isupper()and len(t)<100 and not re.match(r'^\d+[.-]',t):return True
    return any(k in t.upper()for k in['ÍNDICE','CONCLUSIÓN','INTRODUCCIÓN','RESUMEN'])and len(t)<60

def is_title_level_2(t,s):
    if s in['Heading 2','Heading 3']:return True
    if re.match(r'^\d+(\.\d+)*[.-]\s',t):return True
    return len(t)<80 and t.endswith(':')and not t.isupper()

def is_list_item(t):
    return re.match(r'^[•\-–—]\s',t)or re.match(r'^[a-z]\)|[ivxIVX]+\)|\d+\)',t)

def create_footer_simple(section):
    footer=section.footer
    footer.is_linked_to_previous=False
    for p in footer.paragraphs:p.clear()
    p_logo=footer.paragraphs[0]
    p_logo.alignment=WD_ALIGN_PARAGRAPH.LEFT
    r_logo=p_logo.add_run()
    try:
        logo_bytes=base64.b64decode(LOGO_BASE64)
        logo_stream=BytesIO(logo_bytes)
        r_logo.add_picture(logo_stream,width=Cm(1.5))
    except:pass
    p_empresa=footer.add_paragraph()
    p_empresa.alignment=WD_ALIGN_PARAGRAPH.RIGHT
    p_empresa.paragraph_format.space_before=Pt(0)
    p_empresa.paragraph_format.space_after=Pt(0)
    r_emp=p_empresa.add_run("ARQUITECTURA Y GESTIÓN DE OPERACIONES INMOBILIARIAS, S.L.P.")
    r_emp.font.name='Century Gothic'
    r_emp.font.size=Pt(9)
    r_emp.font.bold=True
    r_emp.font.color.rgb=RGBColor(0,0,0)
    p_contacto=footer.add_paragraph()
    p_contacto.alignment=WD_ALIGN_PARAGRAPH.RIGHT
    p_contacto.paragraph_format.space_before=Pt(1)
    r_con=p_contacto.add_run("AVDA. DE IRLANDA 21, 4º D. 45005 TOLEDO | TLFN. 925 299 300 | www.agoin.es | info@agoin.es")
    r_con.font.name='Century Gothic'
    r_con.font.size=Pt(7)
    r_con.font.color.rgb=RGBColor(102,102,102)

def calculate_stats(doc):
    total_words=0
    total_paras=len(doc.paragraphs)
    for para in doc.paragraphs:
        total_words+=len(para.text.split())
    pages=max(1,total_words//300)
    read_time=max(1,total_words//200)
    return {'words':total_words,'paragraphs':total_paras,'pages':pages,'read_time':read_time}

def generate_realistic_preview(doc,title,location):
    preview=f'<div class="doc-preview">'
    preview+=f'<div class="doc-header"><div class="doc-title">{title or"[TÍTULO]"}</div><div class="doc-location">{location or"[DIRECCIÓN]"}</div></div>'
    preview+='<div class="doc-content">'
    count=0
    for para in doc.paragraphs:
        text=para.text.strip()
        if not text or count>=15:continue
        style=para.style.name if hasattr(para,'style')else''
        if is_title_level_1(text,style):
            preview+=f'<h1>{text}</h1>'
        elif is_title_level_2(text,style):
            preview+=f'<h2>{text}</h2>'
        else:
            preview+=f'<p>{text[:300]}</p>'
        count+=1
    if len(doc.paragraphs)>15:
        preview+=f'<p style="color:#999;font-style:italic;text-align:center;margin-top:20px">... y {len(doc.paragraphs)-15} párrafos más</p>'
    preview+='</div>'
    preview+=f'<div class="doc-footer"><img src="data:image/jpeg;base64,{LOGO_BASE64}" width="50"><div class="doc-footer-text"><div class="company">ARQUITECTURA Y GESTIÓN DE OPERACIONES INMOBILIARIAS, S.L.P.</div><div class="contact">AVDA. DE IRLANDA 21, 4º D. 45005 TOLEDO | TLFN. 925 299 300 | www.agoin.es | info@agoin.es</div></div></div>'
    preview+='</div>'
    return preview

def apply_agoin_format_final(input_doc,project_title,project_location,is_text_only=False):
    output_doc=Document()
    for section in output_doc.sections:
        section.top_margin=Cm(2.5)
        section.bottom_margin=Cm(2.5)
        section.left_margin=Cm(3.0)
        section.right_margin=Cm(3.0)
        header=section.header
        header.is_linked_to_previous=False
        for p in header.paragraphs:p.clear()
        header_title=header.paragraphs[0]
        add_green_header_paragraph(header_title,project_title if project_title else"[TÍTULO]",True)
        header_location=header.add_paragraph()
        add_green_header_paragraph(header_location,project_location if project_location else"[DIRECCIÓN]",False)
        header_location.paragraph_format.space_before=Pt(1)
        create_footer_simple(section)
    if is_text_only:
        for line in input_doc.split('\n'):
            line=line.strip()
            if not line:continue
            new_para=output_doc.add_paragraph()
            if is_title_level_1(line,''):
                add_green_header_paragraph(new_para,line,True)
            elif is_title_level_2(line,''):
                r=new_para.add_run(line)
                r.font.name='Century Gothic'
                r.font.size=Pt(11)
                r.font.bold=True
                r.font.color.rgb=RGBColor(0,0,0)
                new_para.alignment=WD_ALIGN_PARAGRAPH.LEFT
                new_para.paragraph_format.space_before=Pt(12)
                new_para.paragraph_format.space_after=Pt(6)
            elif is_list_item(line):
                r=new_para.add_run(line)
                r.font.name='Century Gothic'
                r.font.size=Pt(10)
                new_para.alignment=WD_ALIGN_PARAGRAPH.LEFT
                new_para.paragraph_format.left_indent=Cm(1)
            else:
                r=new_para.add_run(line)
                r.font.name='Century Gothic'
                r.font.size=Pt(10)
                new_para.alignment=WD_ALIGN_PARAGRAPH.JUSTIFY
                new_para.paragraph_format.line_spacing=1.15
    else:
        for para in input_doc.paragraphs:
            if not para.text.strip():continue
            new_para=output_doc.add_paragraph()
            texto=para.text.strip()
            style_name=para.style.name
            if is_title_level_1(texto,style_name):
                add_green_header_paragraph(new_para,texto,True)
            elif is_title_level_2(texto,style_name):
                r=new_para.add_run(texto)
                r.font.name='Century Gothic'
                r.font.size=Pt(11)
                r.font.bold=True
                r.font.color.rgb=RGBColor(0,0,0)
                new_para.alignment=WD_ALIGN_PARAGRAPH.LEFT
                new_para.paragraph_format.space_before=Pt(12)
                new_para.paragraph_format.space_after=Pt(6)
            elif is_list_item(texto):
                r=new_para.add_run(texto)
                r.font.name='Century Gothic'
                r.font.size=Pt(10)
                new_para.alignment=WD_ALIGN_PARAGRAPH.LEFT
                new_para.paragraph_format.left_indent=Cm(1)
            else:
                for run in para.runs:
                    new_run=new_para.add_run(run.text)
                    new_run.font.name='Century Gothic'
                    new_run.font.size=Pt(10)
                    new_run.font.color.rgb=RGBColor(0,0,0)
                    if run.bold:new_run.bold=True
                    if run.italic:new_run.italic=True
                    if run.underline:new_run.underline=True
                new_para.alignment=WD_ALIGN_PARAGRAPH.JUSTIFY
                new_para.paragraph_format.line_spacing=1.15
        for table in input_doc.tables:
            new_table=output_doc.add_table(rows=len(table.rows),cols=len(table.columns))
            new_table.style='Table Grid'
            for i,row in enumerate(table.rows):
                for j,cell in enumerate(row.cells):
                    new_table.rows[i].cells[j].text=cell.text
                    for paragraph in new_table.rows[i].cells[j].paragraphs:
                        for run in paragraph.runs:
                            run.font.name='Century Gothic'
                            run.font.size=Pt(9)
    return output_doc

tab1,tab2=st.tabs(["📂 Subir Archivo","📝 Pegar Texto"])
doc_content=None
is_text_mode=False
source_name="documento"

with tab1:
    uploaded_file=st.file_uploader("Selecciona un archivo DOCX o TXT",type=['docx','txt'])
    if uploaded_file:
        try:
            ext=uploaded_file.name.split('.')[-1].lower()
            if ext=='txt':
                doc_content=uploaded_file.read().decode('utf-8',errors='ignore')
                is_text_mode=True
            else:
                doc_content=Document(uploaded_file)
                is_text_mode=False
            source_name=uploaded_file.name.rsplit('.',1)[0]
            st.success(f"✅ Archivo cargado: {uploaded_file.name}")
        except Exception as e:
            st.error(f"❌ Error al cargar: {str(e)}")

with tab2:
    pasted_text=st.text_area("Pega aquí el texto del documento",height=300,placeholder="Pega tu texto aquí...")
    if pasted_text and len(pasted_text.strip())>10:
        doc_content=pasted_text
        is_text_mode=True
        source_name="texto_pegado"
        st.success(f"✅ Texto detectado: {len(pasted_text)} caracteres")

if doc_content:
    st.markdown("---")
    try:
        info=extract_project_info_from_text(doc_content)if is_text_mode else extract_project_info(doc_content)
        col_a,col_b=st.columns(2)
        with col_a:
            project_title=st.text_input("📌 Título del proyecto",value=info['title'],placeholder="Ej: INFORME TÉCNICO...")
        with col_b:
            project_location=st.text_area("📍 Dirección/Ubicación",value=info['location'],height=80,placeholder="Ej: CALLE MAYOR 123...")

        col1,col2=st.columns([3,1])
        with col1:
            gen_btn=st.button("✨ Generar Documento",use_container_width=True,type="primary")
        with col2:
            if st.button("🔄 Nuevo",use_container_width=True):
                for key in list(st.session_state.keys()):
                    del st.session_state[key]
                st.rerun()

        if gen_btn:
            if not project_title or not project_location:
                st.warning("⚠️ Por favor completa el título y la dirección")
            else:
                with st.spinner("🔄 Generando documento..."):
                    try:
                        output_doc=apply_agoin_format_final(doc_content,project_title,project_location,is_text_mode)
                        buffer_docx=io.BytesIO()
                        output_doc.save(buffer_docx)
                        buffer_docx.seek(0)
                        st.session_state['docx_buffer']=buffer_docx.getvalue()
                        st.session_state['source_name']=source_name
                        st.session_state['output_doc']=output_doc
                        st.session_state['project_title']=project_title
                        st.session_state['project_location']=project_location
                        st.success("✅ Documento generado correctamente")
                        st.rerun()
                    except Exception as e:
                        st.error(f"❌ Error: {str(e)}")

        if 'docx_buffer' in st.session_state:
            st.markdown("---")
            st.markdown("### 📊 Estadísticas del Documento")
            stats=calculate_stats(st.session_state['output_doc'])
            col1,col2,col3,col4=st.columns(4)
            with col1:
                st.markdown(f'<div class="stats-card"><div class="stats-number">{stats["words"]}</div><div class="stats-label">Palabras</div></div>',unsafe_allow_html=True)
            with col2:
                st.markdown(f'<div class="stats-card"><div class="stats-number">{stats["paragraphs"]}</div><div class="stats-label">Párrafos</div></div>',unsafe_allow_html=True)
            with col3:
                st.markdown(f'<div class="stats-card"><div class="stats-number">{stats["pages"]}</div><div class="stats-label">Páginas (est.)</div></div>',unsafe_allow_html=True)
            with col4:
                st.markdown(f'<div class="stats-card"><div class="stats-number">{stats["read_time"]} min</div><div class="stats-label">Lectura</div></div>',unsafe_allow_html=True)

            st.markdown("### 📄 Vista Previa del Documento")
            preview_html=generate_realistic_preview(st.session_state['output_doc'],st.session_state['project_title'],st.session_state['project_location'])
            st.markdown(preview_html,unsafe_allow_html=True)

            st.markdown("### 💾 Descargar Documento")
            col1,col2=st.columns(2)
            with col1:
                st.download_button("📄 Descargar DOCX",data=st.session_state['docx_buffer'],file_name=f"AGOIN_{st.session_state['source_name']}.docx",mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",use_container_width=True)
            with col2:
                st.info("💡 Para PDF: Abre el DOCX en Word y guarda como PDF")

    except Exception as e:
        st.error(f"❌ Error: {str(e)}")
else:
    st.info("👆 Sube un archivo o pega texto para comenzar")

st.markdown("---")
st.markdown('<p style="text-align:center;color:white;font-weight:600;font-size:0.9rem">AGOIN v24 • Formateador Profesional</p>',unsafe_allow_html=True)
