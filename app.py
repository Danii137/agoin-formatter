...
<style>
    .stApp {
        background: linear-gradient(135deg, #1a5c4d 0%, #2d8b73 15%, #ffffff 100%);
    }
    .main-header {
...
with col2:
    st.markdown('<div class="info-box"><h4>✅ Formato AGOIN</h4><p>✓ Títulos verdes</p><p>✓ Alineado derecha</p><p>✓ Logo en pie</p></div>', unsafe_allow_html=True)
    ...
    try:
        ...
        if st.button("✨ Convertir", use_container_width=True):
            with st.spinner("Formateando..."):
                ...
                buffer.seek(0)
                st.success("✅ ¡Listo!")
                st.download_button("📥 Descargar", buffer, f"AGOIN_{uploaded_file.name.rsplit('.', 1)[0]}.docx", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", use_container_width=True)
    except Exception as e:
        st.error(f"Error: {str(e)}")
else:
    st.markdown('<div class="info-box" style="text-align: center; padding: 3rem;"><h3 style="color: #1a5c4d;">👆 Sube un documento</h3></div>', unsafe_allow_html=True)

st.markdown("---")
st.markdown('<div style="text-align: center; color: #666; padding: 2rem;"><p style="font-weight: 600; color: #1a5c4d;">AGOIN Formateador v5.0</p><p>© 2025 AGOIN S.L.P.</p></div>', unsafe_allow_html=True)
