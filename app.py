...
st.set_page_config(page_title="AGOIN - Formateador Profesional", page_icon="🏢", layout="wide")
...
.feature-badge {background-color: #2d8b73; color: white; padding: 0.3rem 0.6rem; border-radius: 3px; font-size: 0.8rem; display: inline-block; margin: 0.2rem;}
</style>
""", unsafe_allow_html=True)

logo_html = f'<img src="data:image/jpeg;base64,{LOGO_BASE64}" width="100" style="margin-bottom: 1rem;">'
st.markdown(f"""
<div class="main-header">
{logo_html}
<h1>📄 AGOIN - Formateador Profesional</h1>
<p>Conversión inteligente al formato corporativo AGOIN</p>
<div><span class="feature-badge">✓ Multi-formato</span><span class="feature-badge">✓ IA Estructural</span><span class="feature-badge">✓ Century Gothic</span><span class="feature-badge">✓ Con Logo</span></div>
</div>
""", unsafe_allow_html=True)
...
def analyze_text_structure(text):
    ...
    for line in lines:
        ...
        if not line_stripped:
            continue
        line_type = {'text': line_stripped, 'level': 0, 'type': 'normal', 'has_numbering': False}
        if re.match(r'^\d+\.\d+\.\d+[.-]', line_stripped):
            line_type['level'] = 3
            line_type['type'] = 'heading'
            line_type['has_numbering'] = True
            ...
            line_type['level'] = 2
            line_type['type'] = 'heading'
        structured_content.append(line_type)
    return structured_content

def extract_project_info(doc):
    project_info = {'title': '', 'location': '', 'section': ''}
    ...
    section_keywords = ['MEMORIA DESCRIPTIVA', 'MEMORIA CONSTRUCTIVA', 'MEMORIA JUSTIFICATIVA', 'CÁLCULO', 'PLIEGO', 'PRESUPUESTO', 'MEDICIONES', 'ANEJO']
...
def add_logo_to_header(header):
    try:
        ...
        run.add_picture(logo_stream, width=Inches(0.8))
        paragraph.alignment = WD_ALIGN_PARAGRAPH.RIGHT
        return True
    except Exception as e:
        st.warning(f"No se pudo añadir el logo: {str(e)}")
        return False

def set_highlight_color(run, color='green'):
    try:
        if color == 'green':
...
def apply_agoin_format(input_doc, project_title, project_location, section_name, is_text_only=False):
    output_doc = Document()
    for section in output_doc.sections:
        ...
        header_para3.text = f"{section_name}         " if section_name else "[SECCIÓN]         "
        header_para3.alignment = WD_ALIGN_PARAGRAPH.LEFT
        for run in header_para3.runs:
            run.font.name = 'Century Gothic'
            run.font.size = Pt(8)
            ...
            if es_titulo:
                ...
                for run in para.runs:
                    ...
                    if hasattr(run.font, 'highlight_color') and run.font.highlight_color:
                        set_highlight_color(new_run, 'green')
                new_para.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
...
with col1:
    ...
    uploaded_file = st.file_uploader("Selecciona un archivo (DOCX, PDF, TXT, DOC)", type=['docx', 'pdf', 'txt', 'doc'], help="Sube el documento que deseas convertir al formato AGOIN")
with col2:
    st.markdown('<div class="info-box"><h4>🎯 Características</h4><p>✓ Múltiples formatos de entrada</p><p>✓ Detección inteligente de estructura</p><p>✓ Preservación de resaltado verde</p><p>✓ Logo AGOIN incluido</p><p>✓ Century Gothic corporativo</p></div>', unsafe_allow_html=True)
...
if uploaded_file is not None:
    try:
        ...
        if doc:
            ...
            with col_a:
                project_title = st.text_area("Título del Proyecto", value=project_info['title'], help="Título que aparecerá en el encabezado", height=100)
                section_name = st.text_input("Nombre de la Sección", value=project_info['section'], help="Tipo de documento o sección")
            with col_b:
                project_location = st.text_area("Ubicación del Proyecto", value=project_info['location'], help="Ubicación que aparecerá en el encabezado", height=100)
            st.markdown("### 🔄 Conversión al Formato AGOIN")
            if st.button("🚀 Convertir con Formato Profesional AGOIN", use_container_width=True):
                with st.spinner("Aplicando formato corporativo inteligente..."):
                    try:
                        output_doc = apply_agoin_format(doc, project_title, project_location, section_name, is_text)
                        output_buffer = io.BytesIO()
                        output_doc.save(output_buffer)
                        output_buffer.seek(0)
                        st.success("✅ ¡Documento formateado exitosamente!")
                        st.info("📋 **Formato aplicado:**\n- ✓ Logo AGOIN en encabezado\n- ✓ Century Gothic corporativo\n- ✓ Estructura inteligente\n- ✓ Resaltado verde preservado\n- ✓ Numeración jerarquizada\n- ✓ Márgenes y espaciado AGOIN")
                        st.download_button(label="📥 Descargar Documento Formateado AGOIN", data=output_buffer, file_name=f"AGOIN_{uploaded_file.name.rsplit('.', 1)[0]}.docx", mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document", use_container_width=True)
                    except Exception as e:
...
st.markdown("---")
st.markdown('<div style="text-align: center; color: #666;"><p><strong>AGOIN Formateador Profesional v2.0</strong></p><p>Arquitectura y Gestión de Operaciones Inmobiliarias S.L.P.</p><p>© 2025 - Formato Corporativo Century Gothic con IA</p></div>', unsafe_allow_html=True)
