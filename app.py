import streamlit as st
from docx import Document
from docx.shared import Pt,Cm,RGBColor
from docx.enum.text import WD_ALIGN_PARAGRAPH
from docx.oxml.shared import OxmlElement,qn
from docxtpl import DocxTemplate
import io,re,base64
from io import BytesIO

st.set_page_config(page_title='AGOIN',page_icon='🏢',layout='wide')
LOGO_BASE64=''

st.markdown("""<style>
.stApp{background:linear-gradient(to top left,#1a5c4d 0%,#2d8b73 20%,#fff 100%)}
.main-header{background:rgba(255,255,255,0.95);padding:2rem;border-radius:15px;text-align:center;margin-bottom:2rem;box-shadow:0 4px 6px rgba(0,0,0,0.1)}
.main-header h1{color:#1a5c4d;margin:0}
.stButton>button{background:linear-gradient(135deg,#1a5c4d 0%,#2d8b73 100%);color:white;font-weight:600;border-radius:10px;padding:0.75rem 2rem}
.preview-box{background:white;padding:2rem;border-radius:10px;border:1px solid #ddd;max-height:500px;overflow-y:auto;font-family:'Century Gothic',sans-serif}
.preview-title{background:#1a5c4d;color:white;padding:8px 15px;margin:10px 0;border-radius:5px;font-weight:bold;font-size:14px}
.preview-subtitle{color:#1a5c4d;font-weight:bold;margin:8px 0;font-size:13px}
.preview-text{text-align:justify;line-height:1.6;margin:5px 0;font-size:12px}
.stats-box{background:white;padding:1.5rem;border-radius:10px;border:1px solid #ddd;margin:1rem 0;text-align:center}
.stat-item{display:inline-block;margin:0 15px;padding:10px}
.stat-number{font-size:24px;font-weight:bold;color:#1a5c4d}
.stat-label{font-size:12px;color:#666}
.template-info{background:#e8f5e9;padding:1rem;border-radius:8px;margin:1rem 0;border-left:4px solid #1a5c4d}
</style>""",unsafe_allow_html=True)

st.markdown(f'<div class="main-header"><img src="data:image/jpeg;base64,{LOGO_BASE64}" width="70"><h1>AGOIN Formatter</h1><p style="color:#2d8b73;margin:0">v26 • Con Plantilla Personalizada</p></div>',unsafe_allow_html=True)

def extract_content_text(doc):
    """Extrae todo el contenido como texto"""
    content=[]
    for para in doc.paragraphs:
        if para.text.strip():
            content.append(para.text.strip())
    return '\n\n'.join(content)

def extract_project_info_from_text(t):
    i={'title':'','location':''}
    l=t.split('\n')[:10]
    for line in l:
        line=line.strip()
        if line and(line.isupper()or any(k in line.upper()for k in['ACTA','INFORME','MEMORIA','PROPUESTA'])):
            if len(line)<100 and not i['title']:
                i['title']=line
                break
    tj=' '.join(l)
    lp=re.search(r'(?:CALLE|AVENIDA|AVDA|C/|AVENIDA DE)[^\n]{0,150}',tj,re.IGNORECASE)
    if lp:i['location']=lp.group(0).strip()
    return i

def extract_project_info(d):
    i={'title':'','location':''}
    if hasattr(d,'paragraphs'):
        for p in d.paragraphs[:5]:
            t=p.text.strip()
            if t and(t.isupper()or any(k in t.upper()for k in['ACTA','INFORME','MEMORIA','PROPUESTA'])):
                if len(t)<100:
                    i['title']=t
                    break
        txt=' '.join([p.text for p in d.paragraphs[:10]])
        lp=re.search(r'(?:CALLE|AVENIDA|AVDA|C/|AVENIDA DE)[^\n]{0,150}',txt,re.IGNORECASE)
        if lp:i['location']=lp.group(0).strip()
    return i

def generate_preview(doc):
    preview=""
    count=0
    for para in doc.paragraphs[:15]:
        text=para.text.strip()
        if text:
            preview+=f"<p class='preview-text'>{text[:300]}{'...'if len(text)>300 else''}</p>"
            count+=1
    if len(doc.paragraphs)>15:
        preview+=f"<p style='color:#999;font-style:italic;text-align:center;margin-top:20px'>... {len(doc.paragraphs)-15} párrafos más</p>"
    return preview if preview else"<p style='text-align:center;color:#999'>Documento vacío</p>"

def get_doc_stats(doc):
    para_count=len([p for p in doc.paragraphs if p.text.strip()])
    word_count=sum(len(p.text.split())for p in doc.paragraphs if p.text.strip())
    pages=max(1,para_count//15)
    return para_count,word_count,pages

with st.sidebar:
    st.image(f"data:image/jpeg;base64,{LOGO_BASE64}",width=100)
    st.markdown("### ⚙️ Configuración")

    mode=st.radio("Modo:",["Sin plantilla","Con plantilla"],index=0)

    if st.button("🔄 Limpiar",use_container_width=True):
        for key in list(st.session_state.keys()):
            del st.session_state[key]
        st.rerun()
    st.markdown("---")
    if mode=="Con plantilla":
        st.markdown("**Variables disponibles:**")
        st.code("{{titulo}}\n{{direccion}}\n{{contenido}}")
    else:
        st.markdown("**Formato:**")
        st.markdown("✓ Encabezados verdes\n✓ Century Gothic\n✓ Logo en pie")

# MODO CON PLANTILLA
if mode=="Con plantilla":
    st.markdown("### 📋 Modo Plantilla")
    st.markdown('<div class="template-info">📝 <strong>Sube una plantilla .docx</strong> con tu formato personalizado (logo fijo, colores, etc.) y usa variables como {{titulo}}, {{direccion}}, {{contenido}} donde quieras insertar los datos dinámicos.</div>',unsafe_allow_html=True)

    col_t1,col_t2=st.columns(2)
    with col_t1:
        template_file=st.file_uploader("📄 Plantilla DOCX",type=['docx'],help="Plantilla con {{variables}}")
    with col_t2:
        content_file=st.file_uploader("📄 Contenido DOCX/TXT",type=['docx','txt'],help="Documento con el contenido")

    if template_file and content_file:
        try:
            # Cargar plantilla
            template_bytes=BytesIO(template_file.read())
            template=DocxTemplate(template_bytes)
            st.success(f"✅ Plantilla: {template_file.name}")

            # Cargar contenido
            ext=content_file.name.split('.')[-1].lower()
            if ext=='txt':
                content_text=content_file.read().decode('utf-8',errors='ignore')
            else:
                content_doc=Document(content_file)
                content_text=extract_content_text(content_doc)

            info=extract_project_info_from_text(content_text)
            st.success(f"✅ Contenido: {content_file.name}")

            st.markdown("### 📝 Datos del Documento")
            col_a,col_b=st.columns(2)
            with col_a:
                titulo=st.text_input("📌 Título",value=info['title'])
            with col_b:
                direccion=st.text_area("📍 Dirección",value=info['location'],height=80)

            if st.button("✨ Generar con Plantilla",use_container_width=True,type="primary"):
                with st.spinner("🔄 Rellenando plantilla..."):
                    try:
                        # Rellenar variables
                        context={
                            'titulo':titulo,
                            'direccion':direccion,
                            'contenido':content_text
                        }
                        template.render(context)

                        # Guardar
                        output_buffer=BytesIO()
                        template.save(output_buffer)
                        output_buffer.seek(0)

                        st.session_state['docx_buffer']=output_buffer.getvalue()
                        st.session_state['source_name']=template_file.name.rsplit('.',1)[0]
                        st.session_state['output_doc']=Document(BytesIO(st.session_state['docx_buffer']))

                        st.success("✅ Documento generado con plantilla")
                        st.balloons()
                        st.rerun()
                    except Exception as e:
                        st.error(f"❌ Error: {str(e)}")
                        st.info("💡 Verifica que la plantilla tenga variables correctas: {{titulo}}, {{direccion}}, {{contenido}}")

        except Exception as e:
            st.error(f"❌ {str(e)}")

# MODO SIN PLANTILLA (V24 original)
else:
    from docx.oxml.shared import OxmlElement,qn

    def add_green_header_paragraph(p,t,b=True):
        r=p.add_run(t)
        r.font.name='Century Gothic'
        r.font.size=Pt(11)
        r.font.bold=b
        r.font.color.rgb=RGBColor(255,255,255)
        p.alignment=WD_ALIGN_PARAGRAPH.RIGHT
        s=OxmlElement('w:shd')
        s.set(qn('w:fill'),'1a5c4d')
        p._element.get_or_add_pPr().append(s)
        p.paragraph_format.space_before=Pt(0)
        p.paragraph_format.space_after=Pt(0)

    def create_footer_simple(section):
        footer=section.footer
        footer.is_linked_to_previous=False
        for p in footer.paragraphs:p.clear()
        p_logo=footer.paragraphs[0]
        p_logo.alignment=WD_ALIGN_PARAGRAPH.LEFT
        r_logo=p_logo.add_run()
        try:
            logo_bytes=base64.b64decode(LOGO_BASE64)
            logo_stream=BytesIO(logo_bytes)
            r_logo.add_picture(logo_stream,width=Cm(1.5))
        except:pass
        p_empresa=footer.add_paragraph()
        p_empresa.alignment=WD_ALIGN_PARAGRAPH.RIGHT
        p_empresa.paragraph_format.space_before=Pt(0)
        p_empresa.paragraph_format.space_after=Pt(0)
        r_emp=p_empresa.add_run("ARQUITECTURA Y GESTIÓN DE OPERACIONES INMOBILIARIAS, S.L.P.")
        r_emp.font.name='Century Gothic'
        r_emp.font.size=Pt(9)
        r_emp.font.bold=True
        r_emp.font.color.rgb=RGBColor(0,0,0)
        p_contacto=footer.add_paragraph()
        p_contacto.alignment=WD_ALIGN_PARAGRAPH.RIGHT
        p_contacto.paragraph_format.space_before=Pt(1)
        r_con=p_contacto.add_run("AVDA. DE IRLANDA 21, 4º D. 45005 TOLEDO | TLFN. 925 299 300 | www.agoin.es | info@agoin.es")
        r_con.font.name='Century Gothic'
        r_con.font.size=Pt(7)
        r_con.font.color.rgb=RGBColor(102,102,102)

    def apply_agoin_format(input_doc,titulo,direccion,is_text):
        output_doc=Document()
        for section in output_doc.sections:
            section.top_margin=Cm(2.5)
            section.bottom_margin=Cm(2.5)
            section.left_margin=Cm(3.0)
            section.right_margin=Cm(3.0)
            header=section.header
            header.is_linked_to_previous=False
            for p in header.paragraphs:p.clear()
            header_title=header.paragraphs[0]
            add_green_header_paragraph(header_title,titulo if titulo else"[TÍTULO]",True)
            header_location=header.add_paragraph()
            add_green_header_paragraph(header_location,direccion if direccion else"[DIRECCIÓN]",False)
            header_location.paragraph_format.space_before=Pt(1)
            create_footer_simple(section)

        if is_text:
            for line in input_doc.split('\n'):
                if line.strip():
                    output_doc.add_paragraph(line.strip())
        else:
            for para in input_doc.paragraphs:
                if para.text.strip():
                    new_para=output_doc.add_paragraph()
                    for run in para.runs:
                        new_run=new_para.add_run(run.text)
                        new_run.font.name='Century Gothic'
                        new_run.font.size=Pt(10)
                        if run.bold:new_run.bold=True
                        if run.italic:new_run.italic=True
        return output_doc

    st.markdown("### 📋 Modo Estándar")
    tab1,tab2=st.tabs(["📂 Archivo","📝 Texto"])
    doc_content=None
    is_text_mode=False
    source_name="documento"

    with tab1:
        uploaded_file=st.file_uploader("DOCX o TXT",type=['docx','txt'])
        if uploaded_file:
            try:
                ext=uploaded_file.name.split('.')[-1].lower()
                if ext=='txt':
                    doc_content=uploaded_file.read().decode('utf-8',errors='ignore')
                    is_text_mode=True
                else:
                    doc_content=Document(uploaded_file)
                    is_text_mode=False
                source_name=uploaded_file.name.rsplit('.',1)[0]
                st.success(f"✅ {uploaded_file.name}")
            except Exception as e:
                st.error(f"❌ {str(e)}")

    with tab2:
        pasted_text=st.text_area("Pega texto",height=300)
        if pasted_text and len(pasted_text.strip())>10:
            doc_content=pasted_text
            is_text_mode=True
            source_name="texto_pegado"
            st.success(f"✅ {len(pasted_text)} chars")

    if doc_content:
        info=extract_project_info_from_text(doc_content)if is_text_mode else extract_project_info(doc_content)
        col_a,col_b=st.columns(2)
        with col_a:
            titulo=st.text_input("📌 Título",value=info['title'])
        with col_b:
            direccion=st.text_area("📍 Dirección",value=info['location'],height=80)

        if st.button("✨ Generar",use_container_width=True,type="primary"):
            with st.spinner("🔄 Formateando..."):
                try:
                    output_doc=apply_agoin_format(doc_content,titulo,direccion,is_text_mode)
                    buffer=BytesIO()
                    output_doc.save(buffer)
                    buffer.seek(0)
                    st.session_state['docx_buffer']=buffer.getvalue()
                    st.session_state['source_name']=source_name
                    st.session_state['output_doc']=output_doc
                    st.success("✅ Generado")
                    st.balloons()
                    st.rerun()
                except Exception as e:
                    st.error(f"❌ {str(e)}")

# Sección común de descarga
if 'output_doc' in st.session_state:
    st.markdown("---")
    para_count,word_count,pages=get_doc_stats(st.session_state['output_doc'])
    file_size=len(st.session_state['docx_buffer'])/1024

    st.markdown("### 📊 Estadísticas")
    st.markdown(f"""<div class='stats-box'>
<span class='stat-item'><div class='stat-number'>{para_count}</div><div class='stat-label'>Párrafos</div></span>
<span class='stat-item'><div class='stat-number'>{word_count}</div><div class='stat-label'>Palabras</div></span>
<span class='stat-item'><div class='stat-number'>~{pages}</div><div class='stat-label'>Páginas</div></span>
<span class='stat-item'><div class='stat-number'>{file_size:.1f}KB</div><div class='stat-label'>Tamaño</div></span>
</div>""",unsafe_allow_html=True)

    st.markdown("### 📄 Vista Previa")
    preview_html=generate_preview(st.session_state['output_doc'])
    st.markdown(f'<div class="preview-box">{preview_html}</div>',unsafe_allow_html=True)

    st.markdown("### 💾 Descargar")
    st.download_button("📄 Descargar DOCX",data=st.session_state['docx_buffer'],file_name=f"AGOIN_{st.session_state['source_name']}.docx",mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",use_container_width=True)

st.markdown("---")
st.markdown('<p style="text-align:center;color:#1a5c4d;font-weight:600">AGOIN v26 • Modo Plantilla + Estándar</p>',unsafe_allow_html=True)
