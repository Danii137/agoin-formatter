...
st.markdown("""<style>.stApp{background:linear-gradient(to top left,#1a5c4d 0%,#2d8b73 20%,#fff 100%)}.main-header{background:rgba(255,255,255,0.95);padding:2rem;border-radius:15px;text-align:center;margin-bottom:2rem}.main-header h1{color:#1a5c4d}.stButton>button{background:linear-gradient(135deg,#1a5c4d 0%,#2d8b73 100%);color:white;font-weight:600;border-radius:10px;padding:0.75rem 2rem}.preview-box{background:white;padding:2rem;border-radius:10px;border:1px solid #ddd;max-height:400px;overflow-y:auto}</style>""",unsafe_allow_html=True)
st.markdown(f'<div class="main-header"><img src="data:image/jpeg;base64,{LOGO_BASE64}" width="70"><h1>AGOIN</h1><p style="color:#2d8b73">v22 • Vista previa + PDF</p></div>',unsafe_allow_html=True)
...
def generate_preview(doc):
    ...
    for para in doc.paragraphs:
        ...
        if text and count<20:
            preview+=f"<p style='margin:5px 0;font-size:14px'>{text[:200]}</p>"
            count+=1
    if len(doc.paragraphs)>20:
        preview+=f"<p style='color:#999;font-style:italic;margin-top:20px'>... y {len(doc.paragraphs)-20} párrafos más</p>"
    return preview if preview else"<p>Documento vacío</p>"
...
with tab2:
    pasted_text=st.text_area("Texto",height=300)
    ...
    try:
        ...
        if 'docx_buffer' in st.session_state:
                    ...
                    with st.spinner("🔄 PDF..."):
                        try:
                            with tempfile.NamedTemporaryFile(delete=False,suffix='.docx')as tmp_docx:
                                tmp_docx.write(st.session_state['docx_buffer'])
                                tmp_docx_path=tmp_docx.name
                            tmp_pdf_path=tmp_docx_path.replace('.docx','.pdf')
                            convert(tmp_docx_path,tmp_pdf_path)
                            with open(tmp_pdf_path,'rb')as f:
                                pdf_bytes=f.read()
                            os_mod.unlink(tmp_docx_path)
                            os_mod.unlink(tmp_pdf_path)
            ...
            if 'pdf_buffer' in st.session_state:
                st.download_button("📑 Descargar PDF",data=st.session_state['pdf_buffer'],file_name=f"AGOIN_{st.session_state['source_name']}.pdf",mime="application/pdf",use_container_width=True)
...
st.markdown("---")
st.markdown('<p style="text-align:center;color:#1a5c4d;font-weight:600">AGOIN v22 FINAL • Vista previa + PDF</p>',unsafe_allow_html=True)
